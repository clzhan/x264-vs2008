...
/* This just improves encoder performance(性能), it's not part of the spec */
/*
16*16参考块mv预测
*/
...
#undef SET_MVP
    ...
    /* temporal(暂存的,短暂的) predictors(预测器；预测者；预测值) */
    if( h->fref0[0]->i_ref[0] > 0 )//h->fref0[] 是指的参考列表 list0 中具体某一幅图像，h->fref0[]->i_ref[0] 则是表示该图像的参考图像在参考列表中的索引号//http://bbs.chinavideo.org/viewthread.php?tid=12685&extra=page%3D1
    {								//fref 第一个f估计是frame 
...
void x264_macroblock_cache_save( x264_t *h )
{
    ...
    const int s8x8 = h->mb.i_b8_stride;/*图像是二维的，代码中是一维数组存储的，这个stride就是第一个维度的大小，可以方便根据二维位置去获得在以一维数组中的索引值。x264中有很多，h->mb.i_b4_stride，就是一行中4x4块得个数，h->mb.i_b8_stride，就是一行中8x8块得个数。16*16块中，stride就是16,8*8块中stride就是8，还有很多，在具体语境中看含义是什么。*/
    const int s4x4 = h->mb.i_b4_stride;
