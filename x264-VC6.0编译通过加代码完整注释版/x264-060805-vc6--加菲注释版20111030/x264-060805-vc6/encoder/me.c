...
void x264_me_search_ref( x264_t *h, x264_me_t *m, int (*mvc)[2], int i_mvc, int *p_halfpel_thresh )
{
    ...
    /* try extra(额外) predictors(预测器) if provided(规定) */
...
me_hex2:
        ...
        if( dir != -2 )//这里用dir=-2作为预测方向的初始值，可以改成其他任意值吗，即为什么取-2呢？//你可以改成其他值，看看编码结果有什么不同，再追查一下为什么不同//http://bbs.chinavideo.org/viewthread.php?tid=12703&extra=page%3D2
        {
            static const int hex2[8][2] = {{-1,-2}, {-2,0}, {-1,2}, {1,2}, {2,0}, {1,-2}, {-1,-2}, {-2,0}};//为什么要取8个点呢？{-1,-2}, {-2,0},这2个点为什么取2次呢，{-1,-2}, {-2,0}, {-1,2}, {1,2}, {2,0}, {1,-2}, 这6个点已经取完6变形的点了?//估计是为了程序设计需要。因为六边形只有 6 个点，如果不这样做，那么在程序中就要判断当前是否是最后一个点。如果不判断，那么 hex2[odir+2] 这样的操作可能就会导致数组越界。//http://bbs.chinavideo.org/viewthread.php?tid=12703&extra=page%3D2
            bmx += hex2[dir+1][0];	//hex:六角形的,十六进制
            bmy += hex2[dir+1][1];
