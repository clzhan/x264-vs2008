...
typedef struct
{
    /* conduct(指导；经营；处理；安排) the analysis using this lamda and QP */
    int i_lambda;
    int i_lambda2;
    int i_qp;
    int16_t *p_cost_mv;
    ...
    /* I: Intra part */
    /* Take some shortcuts in intra search if intra is deemed(被视为) unlikely(不太可能的) */
    int b_fast_intra;
    int b_try_pskip;
    ...
    /* Chroma(色度) part */
    int i_satd_i8x8chroma;//SATD即将残差经哈德曼变换的4×4块的预测残差绝对值总和，可以将其看作简单的时频变换，其值在一定程度上可以反映生成码流的大小。因此，不用率失真最优化时，可将其作为模式选择的依据
    ...
    int i_cost16x16bi; /* used the same ref and mv as l0 and l1 (at least最小的 for now) */
...
3686,4645,5852,7373			   /* 48-51 */
};

/* TODO: calculate CABAC costs 
calculate:计算, 估计, 核算,预测；推测
*/
...
/*
 initialize(初始化) an array(数组, 阵列) of lambda*nbits for all possible(可能的) mvs 
...
static void x264_mb_analyse_load_costs( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    if( !p_cost_mv[a->i_qp] )
    {
        ...
        /* factor(因子) of 4 from qpel, 2 from sign, and 2 because mv can be opposite(对应的) from mvp */
        int i;
...
{
    memset( a, 0, sizeof( x264_mb_analysis_t ) );//初始化结构体对象的内存

    /* conduct(控制, 指导，管理，经营；进行) the analysis using this lamda and QP */
    a->i_qp = h->mb.i_qp = i_qp;
    a->i_lambda = i_qp0_cost_table[i_qp];//lambda:希腊字母的第 11个字
    ...
    /* II: Inter part P/B frame (帧间部分 P/B帧)*/
...
#undef CLIP_FMV
        ...
        /* Fast intra decision(判断,选定) 快速帧内选定*/
...
static void predict_16x16_mode_available( unsigned int i_neighbour, int *mode, int *pi_count )
{
    ...
    else if( i_neighbour & MB_LEFT )//左可用
    {
        /* left available(可用的)*/
    ...
    else if( i_neighbour & MB_TOP )//上可用
    {
        /* top available(可用的)*/
...
static void x264_mb_analyse_inter_p16x16( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    int i_ref;
    int mvc[7][2], i_mvc;
    int i_halfpel_thresh = INT_MAX;//thresh:反复地做
	...
	*/
    ...
    for( i_ref = 0; i_ref < h->i_ref0; i_ref++ )//从第一个参考帧开始
    {
        ...
        x264_mb_predict_mv_16x16( h, 0, i_ref, m.mvp );//调用x264_mb_predict_mv_16x16函数，以上、右上、左块运动矢量的中值m.mvp作为候选运动矢量
        x264_mb_predict_mv_ref16x16( h, 0, i_ref, mvc, &i_mvc );//调用x264_mb_predict_mv_ref16x16函数，寻找其它候选运动矢量。这些候选者包括:空间相邻的左、左上、上、右上块的MV；第0个参考帧中的当前块、右边块、下边快运动矢量乘以时间差权重
        x264_me_search_ref( h, &m, mvc, i_mvc, p_halfpel_thresh );//调用x264_me_search_ref进行运动搜索。搜索时先从所有候选运动矢量中选出最佳的起点，然后使用小钻石法、六边形法、UMH或者全搜索搜索出最佳的整像素位置。
								//x264_me_search_ref调用refine_subpel进行1/2和1/4运动搜索。两者都使用小钻石法
								//搜索出最佳运动矢量后，如果当前是最近一个参考帧，而且最佳SA(T)D小与检测门限，则尝试对其进行P_SKIP编码
								//保存搜索结果
        /* early termination(结束)
         * SSD threshold(阈值的,门槛) would probably be better than SATD */
        ...
        /* save mv for predicting neighbors(邻元素) */
        a->l0.mvc[i_ref][0][0] =
 ...
 * x264_macroblock_analyse:  analyse：解析；macroblock：宏模块
   macroblock的一些说明.doc里有更详细//http://wmnmtm.blog.163.com/blog/static/382457142011773583805/
 * 宏块分析
 *****************************************************************************/
void x264_macroblock_analyse( x264_t *h )//在此处通过计算一系列Mv的SAD值算出最优化的方案，这些包括帧内和帧间的SAD值的计算，比较大小，得到最好的宏块模式
{
    x264_mb_analysis_t analysis;
    int i_cost = COST_MAX;//cost:代价
    int i;

    /* init(初始化) analysis(分析) */
    x264_mb_analyse_init( h, &analysis, x264_ratecontrol_qp( h ) );//主要是初始化变量a，如,帧间，帧内的satd值，初始化MV的范围等一些参数。


	//开始对各个类型帧进行分析
    /*--------------------------- Do the analysis ---------------------------*/
    if( h->sh.i_type == SLICE_TYPE_I )//:::::::::::::::::I:::当是I帧
    {
        x264_mb_analyse_intra( h, &analysis, COST_MAX );//进行帧内预测
        if( analysis.b_mbrd )
    ...
    else if( h->sh.i_type == SLICE_TYPE_P )//:::::::::::::::::P
    {
        int b_skip = 0;
        int i_intra_cost, i_intra_type;

        /* Fast P_SKIP detection(探测) */
        ...
        else
        {
			...
			//当不采用p_skip模式时，则调用如下函数，进行帧间16*16块的分析,计算p16x16模式的运动适量及cost（其中还要考虑到参考帧部分，运动估计）。然后在允许16*16块分割时，进一 步分析8*8，4*4，16*8，8*16块的运动矢量，并比较cost的大小。获得最佳的运动估计模式。得到子宏块的分割方式存储在 i_partion。随后根据i_partion进行像素细化。
            ...
            /* Select best inter mode (选择最佳帧间模式)*/
            i_type = P_L0;
            i_partition = D_16x16;
    ...
    else if( h->sh.i_type == SLICE_TYPE_B )//:::::::::::::::::B::::当是B帧时
    {
        ...
        if( h->mb.b_direct_auto_write )
        {
            ...
            for( i = 0; i < 2; i++ )
            {
                ...
                analysis.b_direct_available = x264_mb_predict_mv_direct16x16( h, i && analysis.b_direct_available ? &b_changed : NULL );//判断是否采用B_Direct模式
                if( analysis.b_direct_available )//如果采用直接模式，则分别进行空间和时间的直接预测,这里B帧双向参考，前向和后向的参考和p帧的方向是类似的。B 模式下最低支持的是8*8子宏块
...
SAD与SATD
   ...
   如果不用率失真最优化， 为什么选择SATD＋delta×r（mv，mode）作为模式选择的依据？为什么运动估计中，整象素搜索用SAD，而亚象素用SATD？为什么帧内模式选择要用SATD？ 
   A: 
