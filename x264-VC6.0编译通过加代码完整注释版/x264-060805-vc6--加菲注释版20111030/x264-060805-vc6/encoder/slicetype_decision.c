 ...
 * 参数：
 * 返回：
 * 资料：//这里要注意默认的x264里面是没有B帧的，如果需要用到B帧可以在最初的参数设置的时候
 *		   用语句“--frames 数字”来定义，x264_slicetype_analyse 也只有在定义了“--frames 数字”时候,才会执行(这句好像不大对劲，用和不用生成的文件一样，而且这个参数从字面上看是指定要编码多少帧呀，测试也是这样)
 * 思路：
 * 
*/
