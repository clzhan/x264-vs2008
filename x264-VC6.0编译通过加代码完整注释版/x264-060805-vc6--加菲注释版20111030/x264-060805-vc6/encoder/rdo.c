 ...
 * rdo.c: h264 encoder library (rate-distortion optimization)
   RDO:Rate Distortion Optimization (拉格朗日率失真优化)
   Distortion:扭曲;变形;失真 ,歪曲
   Optimization:优化,最佳化
   Rate:比率, 率,速度；速率
...
/* duplicate (完全一样的) all the writer functions, just calculating(计算) bit(比特) cost(成本)
 * instead of (而不) writing the bitstream.
 * TODO: use these for fast 1st pass too. 
   好像是说，和写函数(可能是指写位流的部分的函数)是一模一样的，但是仅仅是计算比特成本，而不写比特流。
*/
...
{
    int b_transform_bak = h->mb.b_transform_8x8;//transform:变换
    int i_ssd;
    int i_bits;
...
typedef struct {
    ...
    uint8_t cabac_state[10]; //just(仅仅,真正地) the contexts(上下文) relevant(有关的) to coding abs_level_m1
} trellis_node_t;
