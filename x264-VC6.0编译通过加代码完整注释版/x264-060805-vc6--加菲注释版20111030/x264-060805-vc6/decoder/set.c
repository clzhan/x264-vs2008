...
/* return -1 if invalid, else the id 如果错误返回-1，事则返回id*/
int x264_sps_read( bs_t *s, x264_sps_t sps_array[32] )
{
    x264_sps_t *sps;	//定义一个结构体的指针

    int i_profile_idc;		//序列参数集的第1个句法元素
    int i_level_idc;		//序列参数集的第7个句法元素

    int b_constraint_set0;	//序列参数集的第2个句法元素
    int b_constraint_set1;	//序列参数集的第3个句法元素
    int b_constraint_set2;	//序列参数集的第4个句法元素
							//序列参数集的第5个句法元素，这儿没有定义,constraint_set3

    int id;					//序列参数集的第8个句法元素，seq_parameter_set_id
